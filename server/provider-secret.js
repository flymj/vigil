...
const keyPath = path.join(configDirectory, 'provider-secret.key')
const secretPath = path.join(configDirectory, 'provider-secret.json')
...
async function encryptionKey() {
  try {
    ...
    if (existing.length !== 32) throw new Error('Provider secret key is invalid')
    return existing
  } catch (error) {
...
export async function saveProviderApiKey(value) {
  ...
  await writePrivateFile(secretPath, `${JSON.stringify(payload, null, 2)}\n`)
...
export async function loadProviderApiKey() {
  try {
    ...
    if (payload.version !== 1 || payload.algorithm !== 'aes-256-gcm') throw new Error('Provider secret format is unsupported')
...
export async function providerApiKeyConfigured() {
  return Boolean(await loadProviderApiKey())
}
