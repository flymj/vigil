...
export async function snoopPullRequest(settings, ownerValue, repositoryValue, numberValue) {
  ...
  return {
    ...
    commits: commits.map((commit) => ({ sha: commit.sha.slice(0, 12), fullSha: commit.sha, message: trimText(commit.commit?.message, 500), author: commit.author?.login || commit.commit?.author?.name || '', date: commit.commit?.author?.date })),
...
export async function collectRepositoryWindow(settings, owner, repository, range, branch = '') {
  ...
  return {
    ...
    commits: commits.slice(0, 100).map((commit) => ({ sha: commit.sha.slice(0, 12), fullSha: commit.sha, message: trimText(commit.commit?.message?.split('\n')[0], 500), author: commit.author?.login || commit.commit?.author?.name || '', date: commit.commit?.author?.date, url: commit.html_url })),
    hotPullRequests,
